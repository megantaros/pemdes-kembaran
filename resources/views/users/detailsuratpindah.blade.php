            ...
            <div class="container absolute py-14 left-1/2 transform -translate-x-1/2">
                <div class="flex lg:flex-row flex-col gap-4">
                    ...
                    <div class="basis-1/1 bg-white h-auto w-full rounded-lg p-6 shadow-lg">
                        <div class="lg:text-3xl text-2xl">Detail Surat Keterangan Pindah</div>
                        ...
                        <div class="overflow-x-auto">
                            <table class="table w-full tabel-surat border-none">
                                ...
                                <form action="{{ route('keterangan-pindah.update', ['keterangan_pindah' => $data->id_surat_ket_pindah]) }}" enctype="multipart/form-data" method="POST">
                                    @csrf
                                    @method('PUT')
                                    <tr>
                                        <th scope="row">No. KK</th>
                                        <td>
                                            <input type="text" placeholder="Tulis disini..." class="input input-bordered input-info w-full read-only:bg-[#9cb4cc]" value="{{ $data->kk }}" name="kk"/>
                                        </td>
                                    </tr>
                                    <tr>
                                        <th scope="row">Nama Kepala Keluarga</th>
                                        <td>
                                            <input type="text" placeholder="Tulis disini..." class="input input-bordered input-info w-full read-only:bg-[#9cb4cc]" value="{{ $data->nama_kepala_keluarga }}" name="nama_kepala_keluarga"/>
                                        </td>
                                    </tr>
                                    <tr>
                                        <th scope="row">Alasan Pindah</th>
                                        <td>
                                            <select class="select select-primary w-full show_form_lainnya" name="alasan_pindah">
                                    ...
                                    <tr class="form_lainnya">
                                        <th scope="row">Lainnya</th>
                                        <td>
                                            <input type="text" placeholder="Tulis disini..." class="input input-bordered input-info w-full read-only:bg-[#9cb4cc]" value="{{ $data->lainnya }}" name="lainnya"/>
                                        </td>
                                    </tr>
                                    <tr>
                                        <th scope="row">Alamat Tujuan</th>
                                        <td>
                                            <textarea class="textarea textarea-info w-full" placeholder="Tulis disini..." name="alamat_tujuan">{{ $data->alamat_tujuan }}</textarea>
                                        </td>
                                    </tr>
                                    <tr>
                                        <th scope="row">Jumlah Anggota Pindah</th>
                                        <td>
                                            <input type="number" placeholder="Tulis disini..." class="input input-bordered input-info w-full read-only:bg-[#9cb4cc]" value="{{ $data->jml_angg_pindah }}" name="jml_angg_pindah"/>
                                        </td>
                                    </tr>
                                    <tr>
                                        ...
                                        <td>
                                            <select class="select select-primary w-full" name="shdk">
                                                @if($data->shdk == '01')
                                                    <option value="01" selected disabled>Kepala Keluarga</option>
                                                @elseif($data->shdk == '02')
                                                    <option value="02" selected disabled>Suami</option>
                                                @elseif($data->shdk == '03')
                                                    <option value="03" selected disabled>Istri</option>
                                                @elseif($data->shdk == '04')
                                                    <option value="04" selected disabled>Anak</option>
                                                @elseif($data->shdk == '05')
                                                    <option value="05" selected disabled>Menantu</option>
                                                @elseif($data->shdk == '06')
                                                    <option value="06" selected disabled>Cucu</option>
                                                @elseif($data->shdk == '07')
                                                    <option value="07" selected disabled>Orangtua</option>
                                                @elseif($data->shdk == '08')
                                                    <option value="08" selected disabled>Mertua</option>
                                                @elseif($data->shdk == '09')
                                                    <option value="09" selected disabled>Famili Lainnya</option>
                                                @elseif($data->shdk == '10')
                                                    <option value="10" selected disabled>Pembantu</option>
                                                @endif
