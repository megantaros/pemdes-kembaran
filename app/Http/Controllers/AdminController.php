...
class AdminController extends Controller
{
    ...
    public function dashboard(Request $request) {
        if ($request->search) {
            ...
            ->select('warga.*', 'surat_pengajuan.*')
            ->where('surat_pengajuan.id_surat', '!=', NULL)
            // ->latest('surat_pengajuan.created_at')
            ->paginate(10);
        }
        $surat_masuk = DB::table('surat_pengajuan')->where('status', 'Terkirim')->where('surat_pengajuan.id_surat', '!=', NULL)->count();
        $surat_keluar = DB::table('surat_pengajuan')->where('status', 'Diterima')->where('surat_pengajuan.id_surat', '!=', NULL)->count();
        $surat_ditolak = DB::table('surat_pengajuan')->where('status', 'Ditolak')->where('surat_pengajuan.id_surat', '!=', NULL)->count();
            // SuratPengajuan::where('id_surat', NULL)->delete();
